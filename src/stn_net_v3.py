...
class CNN(object):
    def __init__(self, config):
        ...
        subdir = date_time + '_stn_v3_' + hyper_params # this version uses inception v3
        ...
        self.localizer = LocalizerInceptionV3(2, 2, self.batch_size)
        self.logits, penalty = self.add_model(self.x_placeholder, self.is_training_placeholder)
        self.preds, self.accuracy_op = self.predict(self.logits, self.y_placeholder)
        self.loss, self.loss_summary = self.add_loss_op(self.logits, self.y_placeholder, penalty)
    ...
    def add_loss_op(self, logits, y, penalty):
        with tf.name_scope('loss'):
            ...
            loss = tf.reduce_mean(entropy + 1e-7 * penalty, name='loss')
        with tf.name_scope('summaries'):
            tf.summary.scalar('loss', loss)
            tf.summary.scalar('penalty', tf.reduce_mean(penalty))
