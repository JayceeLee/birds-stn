...
class CNN(object):
    ...
    def load_data(self):
        ...
        train_y = tf.one_hot(tf.constant(train_y), self.num_classes)
        test_x  = tf.constant(test_x)
        test_y = tf.one_hot(tf.constant(test_y), self.num_classes)
    ...
    def add_placeholders(self):
        ...
        with tf.name_scope('data'):
            ...
            self.y_placeholder = tf.placeholder(tf.int32, shape=(self.batch_size, self.num_classes), name='labels') # one hots
